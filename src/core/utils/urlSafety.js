 ...
 * and the `safe_url` Liquid filter. Lives under src/core so the filter (also
 * core) can import it without reaching into a backend module.
 *
 * NOTE: server/utils/linkPrefixer.js has a parallel `normalize` used for
 * depth-aware URL *classification* (prefixing + canonicalPath). This copy is the
 * *security* normalizer; both implement the same WHATWG URL preprocessing but
 * serve different layers — keep them in step if the interpretation changes.
 */

# Arch Theme Design Guide

> **Complete reference for creating widgets in the Arch theme**

This document contains everything you need to know when creating a new widget for the Arch theme. Follow these patterns and conventions to ensure consistency and maintainability.
...
- `id="{{ widget.id }}"` - Unique widget instance ID
- `class="widget widget-{name} widget-{{ widget.id }}"` - Base classes + scoped class
...
- **Base font size**: `62.5%` (10px) - so `0.1rem = 1px`
...
**NEVER use hardcoded `font-size`, `font-weight`, `line-height`, or `color` in CSS.** Use `block-text` utilities instead:

#### Size Modifiers

```html
<span class="block-text block-text-xs">Small text</span>
<span class="block-text block-text-sm">Small text</span>
<span class="block-text block-text-base">Base text</span>
<span class="block-text block-text-lg">Large text</span>
...
```html
<span class="block-text block-text-normal">Normal</span>
<span class="block-text block-text-medium">Medium</span>
<span class="block-text block-text-semibold">Semibold</span>
<span class="block-text block-text-bold">Bold</span>
```
...
```html
<h3 class="widget-card-title block-text block-text-xl block-text-bold block-text-heading">
  Card Title
</h3>
<p class="widget-card-description block-text block-text-sm block-text-muted">
  Description text
</p>
```
...
These classes are semantic hooks (no default styling):
...
File location: `themes/arch/assets/widget-name.js`
...
**For detailed standards and audit checklist, see `docs/standards.md`**

...
| `color_scheme` | `standard`, `standard-accent`, `highlight`, `highlight-accent` | `standard` inherits the page background with no extra padding. Accent and highlight variants activate a padded container with tinted or contrasting backgrounds. |
...
- Settings: `image_position: right`, `color_scheme: highlight-accent`, `top_spacing: none`, `logo: client-brand.svg`, `author_image: professional-headshot.jpg`
- Blocks: text (lg) for a two-sentence quote, heading (2xl) for client name, text (sm, muted) for "VP of Marketing, Acme Corp"
- Good for: Landing pages where you lead with proof instead of a traditional hero. The highlight-accent background creates a bold color break.
...
- Settings: `image_position: right`, `color_scheme: standard-accent`, `logo: partner-logo.png`, `author_image: (empty -- use placeholder)`
...
- Settings: `image_position: right`, `color_scheme: highlight-accent`, `logo: event-logo.png`, `author_image: speaker-on-stage.jpg`
...
- Settings: `image_position: left`, `color_scheme: standard-accent`, `logo: (empty)`, `author_image: editorial-portrait.jpg`
...
- **Pair color_scheme with adjacent widgets.** Alternate between `standard` and `highlight` (or their accent variants) to create a visual rhythm down the page. Two consecutive `standard` sections blur together; a `highlight` testimonial between them acts as a divider.
...
- **Remove spacing to create section joins.** Setting both `top_spacing: none` and `bottom_spacing: none` on a `highlight-accent` testimonial sandwiched between two `standard` sections creates a bold color band effect that breaks up long pages without adding empty space.

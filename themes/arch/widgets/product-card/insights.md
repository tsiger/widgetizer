...
| `quick_view` | `true` / `false` (default) | When on, the card image and title open a popup with the full product details — larger image, variant selector, description, and add to cart. |
...
- **Cart styling** rides on the CSS parts Shopify exposes (`shopify-cart::part(...)` rules in `assets/commerce.css`): theme colors, radius tokens, and body font. Parts only go so far — a fully custom cart UI would still be a dedicated widget (a later slice).
- **Variant selectors** are also themed through CSS parts (`::part(radio)`, `::part(label)`, …) at two scales: compact on cards (`.product-card-options`), comfortable in the modal (`.product-detail-options`).
- **Quick view** reuses one global, theme-owned modal: `layout.liquid` renders `snippets/product-modal.liquid` (a native `<dialog>` holding a `wait-for-update` product context that renders `snippets/product-detail.liquid` — the same detail view the Product widget places in-page). The card trigger calls the context's `update(event)` to load the clicked product, wired in `assets/shopify-quick-view.js`; the gallery controls live in `assets/shopify-gallery.js` (both enqueued only when Quick view is on), and Buy now / add-to-cart are global wiring in `assets/shopify-cart.js`. The modal is fixed-height (only the details column scrolls on desktop).

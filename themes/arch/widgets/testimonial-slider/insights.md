...
| `color_scheme` | `standard` / `standard-accent` / `highlight` / `highlight-accent` | `standard` inherits page background with no extra padding. The other three add a padded container and swap `--bg-primary`, giving a distinct band of color behind the quotes |
...
Three 5-star reviews from recognizable local roles (e.g., "Owner, Main St Bakery"). Autoplay on, speed 5000 ms, `standard` color scheme. No avatars needed.
...
Five testimonials, each with a headshot avatar. Autoplay on, speed 6000 ms to give visitors time to register the face. `highlight` color scheme to create a distinct band that draws the eye.
...
One long, detailed testimonial. Autoplay off (only one slide, so pagination dots hide automatically). `highlight-accent` color scheme for maximum contrast. Pair it between a services section and a contact form.
...
Six to eight short one-sentence quotes. Autoplay on, speed 3000 ms so they cycle fast. `standard-accent` scheme with `top_spacing: none` and `bottom_spacing: none` so it sits flush between content blocks like a ticker.
...
Four reviews with explicit star ratings (mix of 4 and 5 stars to feel authentic). Autoplay on, speed 5000 ms. `standard` color scheme so the stars themselves provide the visual punch. Avatars omitted to keep the focus on the rating and the quote text.
...
Two to three testimonials placed directly below a hero widget. `highlight` scheme, `top_spacing: none` so there is no gap after the hero. Autoplay on, speed 7000 ms for a calm cadence. Include avatars and titles.
...
Three testimonials positioned just above the footer. `standard-accent` scheme, `bottom_spacing: none`. Autoplay on, speed 5000 ms. Short punchy quotes, no avatars, 5-star ratings.
...
- **Pair with a color scheme that contrasts the surrounding sections.** If the sections above and below use `standard`, switch the testimonial slider to `highlight` or `highlight-accent` so it reads as a distinct social-proof moment rather than blending in.

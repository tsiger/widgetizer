...
| `color_scheme` | `standard` / `standard-accent` / `highlight` / `highlight-accent` | Controls the section background and overall palette. `standard` has no extra padding; the other three add padded container treatment and override `--widget-bg-color`. |
...
- **Settings**: 2-3 hotspots on a single "after" photo. `heading_alignment: left`, eyebrow "The Transformation", `color_scheme: standard-accent`.
...
- **Settings**: 4-6 hotspots on a beautifully plated spread or table scene. `heading_alignment: center`, eyebrow "Chef's Selection", `color_scheme: highlight-accent`, `top_spacing: none` (to pair tightly with a hero above).
...
- **Settings**: 3-4 hotspots on an infographic-style image or process photo. `heading_alignment: left`, title "What's Included", `color_scheme: standard-accent`.
...
- **Choose the right color scheme for contrast.** If the base image is dark, `highlight` or `highlight-accent` schemes frame the section with a lighter surround, making the pin markers more visible at the edges. For light images, `standard` keeps things clean.

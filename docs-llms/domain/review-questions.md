...
**Status:** OSS implementation reviewed — see [priorities and checkpoints](review-status.md#r7--multilingual-website-output).

**Original observation:** steps 20–21 now select exportable languages, enumerate their collection items, generate multilingual SEO/form artifacts and scope listings/pagination to the rendered language. Focused source/assertion review and test runs cover the main new cases. The earlier root-oriented export observation is superseded. `manifest.collections[].itemCount` remained a default-language count, so not every summary had the same scope as the generated site.

**Question:** do the completed theme controls and upgrade path satisfy these rules in a real project? Also check explicit links into skipped languages, translated item alternates and Markdown, noindex siblings, same-language form collisions/limits, and whether the manifest count's scope is clear to consumers.

**Answered: forms and Markdown were already right; three inventories disagreed about what the export publishes.** Every finding was reproduced through a real export before being fixed, and each has a regression verified to fail without its fix.

**The general shape: deciding not to publish something is only half a decision.** The export refuses to publish a language with no homepage (§7b) and says so — but three other inventories were still built from the project on disk rather than from the site being published.

- **Links into a skipped language.** The renderer loads its uuid→page map itself when nobody supplies one, and that load reads every page in every language. A link to a page in a skipped language therefore resolved to a real-looking href for a file the export never writes. The export now seeds the map from the pages it is publishing, so such a target reads as absent and is cleared exactly like a deleted one. The same applied to collection items, whose map was loaded across all project languages. Because menus, theme settings and the listing index read the same map, they are covered by the same fix. Richtext keeps the anchor's words and drops the destination, per [R2's policy](#r2-one-deletion-policy-for-references). The `LANGUAGE_SKIPPED` warning now says links into the language were removed, so the cleanup is not silent.
- **The manifest's collection summary.** `itemCount` was taken only on the default-language pass while `itemPages` beside it described the whole site — two scopes in one object with nothing marking the difference. The total now spans every exported language and `itemCountByLanguage` breaks it down, which is what makes the total checkable.
- **noindex and hreflang.** A page that asked not to be indexed was still advertised as an hreflang alternate, both in its sibling's sitemap entry and in the sibling's `<link>` tags. No specification forbids this — sitemaps.org and RFC 8288 are silent — but search engines drop such annotations and report the sitemap entry as an error, and the mainstream CMS plugins strip noindex content from sitemaps. Translation entries now carry `noindex`, both emitters skip those entries including `x-default`, and a noindex page publishes no cluster of its own (§7d). The language switcher deliberately ignores the flag: noindex is a crawler directive, and a visitor can still follow the link.

**Two further defects came out of review, and they share one lesson: the HTML and the sitemap read the same fact through different objects, so one of them agreeing is not evidence the other does.** The sitemap reads full item records; the HTML reads item *references* from the uuid map, which carried no SEO fields — so a noindex item translation was excluded from the sitemap and advertised in the HTML. Rather than add the one missing field, the shape itself was made single: `translationSibling` is now the only definition of what a sibling carries, `buildTranslations` narrows both inputs through it, and the reference map is built from it. Separately, a fallback entry points at a language's homepage, and the noindex check was reading the *missing sibling* rather than the homepage it actually points at, so a noindex homepage was still published as `x-default`. `buildTranslations` now keeps the homepage itself, not merely the fact that one exists.

**Confirmed already correct:** form keys are language-qualified, same-language collisions with differing fields fail the export, the distinct-form limit is enforced, and Markdown twins are emitted for every exported page and item and for none of a skipped language.

**Simplification candidate (taken):** the export-language selection is now the single source for the render maps, the collection item map and the manifest summary, so those inventories no longer derive their scope independently. The sibling shape the SEO builders and the renderer disagreed about is now one exported projection, `translationSibling`: `buildTranslations` narrows every record through it before reading, and the uuid reference map is built from it. Both paths now use one definition of the sibling fields, preventing the missing-field mismatch found in this review.

Evidence: [output boundaries](operations/output.md#multilingual-boundary-at-this-snapshot), [contentAddress](../../packages/core/src/utils/contentAddress.js), [exportController](../../packages/builder-server/src/controllers/exportController.js), [translations](../../packages/core/src/utils/translations.js). Coverage: O1/O3.

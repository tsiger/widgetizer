...
Deleting media is refused while its recorded `usedIn` list is nonempty. On an allowed delete, the original and generated assets are removed before the database record. This makes accurate usage and partial asset-deletion behavior important. A usage refresh rebuilds references from content and shared settings; it is not a substitute for verifying every write path.
...
Tests: [media](../../../packages/builder-server/src/tests/media.test.js), [mediaUsage](../../../packages/builder-server/src/tests/mediaUsage.test.js), [collection media](../../../packages/builder-server/src/tests/collectionMediaUsage.test.js), [upload atomicity](../../../packages/builder-server/src/tests/mediaInsertAtomicity.test.js), [image language output](../../../packages/core/src/tags/__tests__/imageTagLanguages.test.js).

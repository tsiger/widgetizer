...
**Status:** design approved 2026-07-12. The concurrency primitive (single-flight `save()`, generation-gated `reset()`, backoff) and the undo/redo integration are implemented in `packages/editor-ui/src/stores/saveStore.js` and `packages/editor-ui/src/components/pageEditor/EditorTopBar.jsx`.
...
1. **Save failures are invisible.** `save()`'s catch block only `console.error`s on any non-`PROJECT_MISMATCH` error and never rethrows; the promise always resolves as if it succeeded. Any caller error handling that depends on that rejection never gets to fire.
...
- **Failure surfacing:** a **manual** save's promise **rejects** on failure (the caller's existing catch-and-log handling fires unchanged). An **autosave** failure does **not** reject — it resolves `{status:'failed'}`, is logged, and retried with backoff. A background autosave failing shouldn't interrupt the user mid-typing every 60s; a user-initiated click failing should tell them so.
...
`save()` returns `{ status: 'clean' | 'success' | 'mismatch' | 'failed' | 'abandoned' }` instead of `undefined`, so callers can finally distinguish a busy no-op from a genuine success. A manual save's promise *rejects* on `'failed'` (not resolves) so the caller's existing catch-and-log handling (e.g. `EditorTopBar`'s manual-save button, which logs the rejection via `console.error`) fires with zero changes there; an autosave's promise resolves with `{status:'failed', error}`. `'abandoned'` always *resolves* (never rejects), for both manual and auto — by the time a generation mismatch is detected the caller's own context (the component that triggered it) is generally already gone, per the discard/navigation flow that triggers it; there is nothing left to usefully show a rejection to.

### Abandoning on discard

`reset()` no longer force-clears `isSaving`/`isAutoSaving` — that was the actual bug (flipping flags while the real network request was still pending). Instead it bumps `saveGeneration`. A save captures its own generation at entry; before applying **any** write-back — on the success path (clearing `modifiedWidgets`, rebaselining `originalPage`/`originalGlobalWidgets`, `temporal.clear()`, etc.) *and* symmetrically at the top of the catch block (`markStale()`/`stopAutoSave()` for `PROJECT_MISMATCH`, the failure-count-driving `{status:'failed'}` for any other error) — it checks the generation still matches, and returns `{status:'abandoned'}` if not. A rejection from an abandoned generation must have zero side effects on current-generation state, exactly like a stale success. Each save's own `finally` block *always* clears its own `isSaving`/`isAutoSaving`/`runningSave` bookkeeping regardless of generation — that part is "this execution finished," decoupled from "should its results apply."
...
`EditorTopBar.jsx`'s `safeUndo`/`safeRedo` gain one new step after the temporal jump: diff every widget id (page widgets + header + footer) between the new current content and the saved baseline (`originalPage`/`originalGlobalWidgets`), and call the *existing* `markWidgetModified`/`markWidgetUnmodified` for whichever ids actually differ. This fixes content-level undo/redo dirty-tracking — `markWidgetModified`'s existing side effect re-arms the timer when redo reintroduces per-widget content dirtiness, and `markWidgetUnmodified` clears the Set when undo reverts a widget back to exactly its saved state. It's per-widget-content diffing only, though: a structure-only redo (only `widgetsOrder`/page-settings/theme reintroduced, no individual widget's own content changed) marks no widget and doesn't re-arm the timer. `hasUnsavedChanges()`'s separate whole-page diff still catches that case (nav guard, manual save both work), and the timer re-arms on the next real edit.
...
| Trigger | Behavior |
|---|---|
| Manual save fails | Promise rejects → caller's existing catch-and-log handling fires (e.g. `EditorTopBar`'s manual-save button) |
...
- All four tasks touch only `packages/editor-ui/src/stores/saveStore.js`, `packages/editor-ui/src/stores/__tests__/saveStore.test.js`, `packages/editor-ui/src/components/pageEditor/EditorTopBar.jsx`, and `packages/editor-ui/src/components/pageEditor/__tests__/EditorTopBar.test.jsx`. No other files change.
  ...
  save: async (isAuto = false) => {
    ...
    const run = (async () => {
      ...
      try {
        ...
        // Manual saves rethrow so the caller (e.g. EditorTopBar's manual-save
        // button) can catch and log the rejection; autosave failures stay silent —
...
Expected: all pass (checks nothing outside this file broke — e.g. `EditorTopBar.test.jsx` calls `save()`/reads `isSaving` and must still behave correctly).

...
- `link`: the URL or page filename (e.g. `about.html` for internal pages, `work/project-alpha.html` for a collection item page, or a full URL for external links). For internal targets this is a denormalized convenience value; the authoritative target is the stable UUID below. At render time the emitted `link` is rebuilt from the uuid: `about.html` / `work/project-alpha.html`, or `about` / `work/project-alpha` when the project's Clean URLs setting is on (home becomes `./`, `../` from an item page). `canonicalPath` always keeps the `.html` file path.
...
- `resolveMenuItemLinks(menuItems, pagesByUuid, outputPathPrefix, collectionItemsByUuid, depth, cleanUrls, defaultLanguage)` recursively rewrites each item's emitted `link` (depth-aware, prefixed via `prefixInternalHref`) plus an un-prefixed `canonicalPath` for active-state matching. A `collectionItemUuid` resolves to the item's current address and a `pageUuid` to the page's current address, in the shape the project's Clean URLs setting picks (`pageHref` / `itemHref`: `slugPrefix/slug.html` and `slug.html`, or extensionless `slugPrefix/slug` and `slug` with home as `./` / `../`); `canonicalPath` always keeps the `.html` file path. The **target's own language** decides its folder (`el/about.html`, clean `el/`), read off the loaded entry and compared against the project's `defaultLanguage`; since `outputPathPrefix` is the rendering page's depth, a cross-language link is plain prefixing. A missing target clears the link (and drops the dead ref in the resolved copy); a custom `link` is passed through `sanitizeHref`. Recursion stops at `MAX_MENU_DEPTH` so a hostile/legacy tree can't blow the stack.
- `resolveMenuPageLinks(menuData, …)` wraps the above over `menuData.items`.
- `resolveMenuSettings(settings, schemaSettings, deps)` is the single source of truth for resolving every `menu`-type widget/block setting into a full menu object the menu snippet can render. It looks the stored menu value up in `menuMaps.byUuid` / `menuMaps.bySlug` and resolves its items; a missing/empty value or unknown menu yields `{ items: [] }`. It is consumed by widget rendering (`renderEngine.js`) and collection-item rendering (`collectionService.js`). `deps` is `{ menuMaps, pagesByUuid, collectionItemsByUuid, outputPathPrefix, cleanUrls }`.

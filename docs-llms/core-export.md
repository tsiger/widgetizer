...
6. **Site icons** — `generateExportSiteIcons(...)` writes favicon/app-icon assets at the export root when `general.favicon` is set (`favicon.svg` for SVG sources, `favicon-32.png`, `apple-touch-icon.png`, `icon-192.png`, `icon-512.png`, and `site.webmanifest`). See [Media Library](core-media.md) and [App Settings](core-appSettings.md).
7. **SEO files** (conditional on `siteUrl`) — `buildSitemap` and `buildRobotsTxt` (`services/seoArtifacts.js`) write `sitemap.xml` and `robots.txt` with absolute URLs. Collection item pages are included via `itemPagesForSeo` (valid items of `hasItemPages` collections, in listing order).

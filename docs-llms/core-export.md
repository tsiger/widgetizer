...
The `ExportSite.jsx` component uses a **component-based architecture** that separates concerns:
...
4.  **Render Global Widgets**:
    - The header and footer widgets are rendered into HTML strings using the `renderingService`. They are re-rendered for each page (inside the page loop) so each page's `sharedGlobals` capture the assets they enqueue. This is done in `"publish"` mode, which ensures that asset paths in the final HTML are relative (e.g., `assets/images/logo.png`) instead of absolute API URLs.
...
6.  **Generate SEO Files**:
    - **`sitemap.xml`** and **`robots.txt`** (conditional on `siteUrl`): If the project has a `siteUrl` defined, static versions are generated with absolute URLs baked in. These are for users who know their deployment URL upfront. (In the code these files are written right after validation passes, before the page render loop.)
...
10. **Write Export Metadata Files**:
    - Each static export directory receives a root **`manifest.json`** metadata file describing the export itself (`generator`, `widgetizerVersion`, `themeId`, `themeVersion`, `exportVersion`, `exportedAt`, `projectName`, plus a `collections` array — see the collections section below).
    ...
    - If the project contains any core form widgets, a root **`widgetizer.forms.json`** forms manifest is written via `buildFormsManifest` (`server/services/formsManifestService.js`); form validation errors throw with a 400 status. See [mvp-oss-form-widget.md](mvp-oss-form-widget.md).
...
- **Two-pass validation (before any disk write)**: the controller loads pages and validated collection schemas/items first; if any `hasItemPages` item is `invalid` (missing required fields), the export **fails with 400** and a full per-item-per-field error list, writing **no** HTML. The same pre-write pass also rejects (400) any `hasItemPages` collection that has renderable items but no `template.liquid`, so a missing template never leaves partial export artifacts behind. Only when validation is clean does any disk write begin (favicons → sitemap/robots → pages → items).
- **Per-item render loop**: each item gets **fresh** `sharedGlobals` (new `enqueuedStyles`/`enqueuedScripts`/`enqueuedPreloads`/`collectionCache` Maps; shared cached `pagesByUuid`/`menuMaps`/`collectionItemsByUuid`; `outputPathPrefix: "../"`; `currentCanonicalPath: "{slugPrefix}/{itemSlug}.html"`) so styles/scripts enqueued by one item don't bleed into the next. Each item is rendered through the shared `renderCollectionItemPage` helper in `renderingService` (also used by the preview path, so export and preview can't drift): it resolves links and sanitizes item settings via `prepareCollectionItemForRender` (resolve-then-sanitize, so resolved hrefs are validated too), renders header/footer with the item's globals, renders the template via `renderLiquidTemplate`, and wraps it via `renderPageLayout` with `contentSections.bodyClass = "collection-{type} item-{slug}"`. A missing `template.liquid` for a `hasItemPages` collection with renderable items is caught in the pre-write validation pass (before any disk write), with a defensive re-check in this render loop.
- **Page-shaped object & SEO**: `buildCollectionItemPageData` builds the `page` object the layout/SeoTag consume from the item's own page-shaped `seo` object (Finding #12 — at parity with page SEO): `robots`, `description`, `og_title`, manual `og_image`, `og_type: "article"`, `twitter_card: "summary"`, and `canonical_url` (explicit author value, else auto-built from `siteUrl + slugPrefix/slug`). `SeoTag` applies the same `<title>`/og/twitter fallbacks it does for pages.

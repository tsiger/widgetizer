...
- **A `noindex` destination is never an alternate, and `x-default` is not exempt.** *(Added 2026-09-20, from the R7 review.)* A cluster is a set of pages a crawler may index, so an annotation pointing into a page that asked not to be indexed is discarded by search engines and leaves the site contradicting itself. The rule applies to a sibling page, a sibling item, and the homepage a `fallback: true` entry points at — which means the destination's own directive decides, not the missing translation's. A `noindex` page emits no set at all, since it cannot carry the self-reference the first rule requires. `page.translations` therefore carries a `noindex` flag beside `fallback`, and §7c's switcher deliberately ignores it: `noindex` speaks to crawlers, and a visitor can still follow the link.

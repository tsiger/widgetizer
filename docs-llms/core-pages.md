...
- **`parentPageUuid`** (optional): The page this one sits under, set with the Parent page picker in Page settings. It exists for [breadcrumbs](theming.md#breadcrumbs) — pages are flat on disk and in their URLs, so this is the only statement of hierarchy. Absent means top level, which is the usual case; the key is dropped entirely when cleared rather than written empty. A page cannot be its own parent (refused server-side as well as in the picker), the picker hides the page's own descendants so a cycle cannot be built, and the trail builder stops at depth 10 regardless. Deleting a page clears the pointer on its children (`cleanupDeletedPageReferences`); duplicating a project remaps it to the copy's new uuids, dropping a parent that belonged to another project.

Breadcrumb hierarchy deliberately does not come from menus: navigation may contain several arrangements, external links and labels without pages, and rearranging it must not silently change a page's trail or search metadata. Flat URLs cannot express the relationship either. Collection items use the [listing anchor or sole listing page](core-collections.md#5a-listing-pages-and-the-anchor); themes control the trail's appearance, while core owns these relationships.
...
This is the core of the backend logic. Every handler resolves `const { scope } = req;` and `const { storage } = req.adapters;`, then reads/writes project-relative keys through the adapter. Page writes are funneled through a shared `persistPageWithMediaTracking` helper that writes the JSON, deletes the previous file on a slug change, and syncs media usage; deletes go through `deletePageWithMediaTracking`.

**Language.** No handler spells a key: `pageKey(slug, lang)` / `pagesDir(lang)` from `@widgetizer/core/contentAddress` do, given `lang = { language, defaultLanguage, languages }` from `requestLanguage(req, res)` (`packages/builder-server/src/utils/contentLanguage.js`). The language comes from `?language=` or `body.language`; absent means the project's default. The default language owns `pages/`; another language owns `pages/<lang>/`. A code the project has not enabled is a `400`, so a folder is never created for one. The folder is the only record of the language: `persistPageWithMediaTracking` strips a `language` field before writing, and every read stamps the resolved code back on the model (`getAllPages` lists the root and each enabled language's folder). Slugs are unique per folder — `contact` may exist once per language — and a root slug equal to an enabled language code is reserved (`isReservedPageSlug(slug, lang)`), since `el.html` and `el/` would share a public address. A page also carries `translationGroupId`: its own `uuid` on create and duplicate (a group of one), preserved on update. The dir-explicit render-path reader `listPagesFromDir(projectDir, { defaultLanguage })` walks the same folders.
...
- **Create (`createPage`)**:
  ...
  3. Derives a slug: if the request supplies a slug it is fed through `generateUniqueSlug` (with a `page` fallback); otherwise the slug is generated from `name`. `generateUniqueSlug` (`utils/slugHelpers.js`) slugifies via `sanitizeSlug` and appends `-1`, `-2`, … until `storage.exists(scope, \`pages/${slug}.json\`)` is false.
     The slug `page` is reserved (it is the pagination folder, `blog/page/2.html`); `pageSlugTaken` treats it as existing, so it becomes `page-1`.
...
- **Update (`updatePage`)**:
  ...
  2. **Explicit slug conflict**: when the user explicitly supplied a non-empty slug that differs from the old one and a file already exists at the new slug, the handler returns **`409`** (`"Slug already exists"`) rather than silently appending a counter. (The empty-slug fallback path is already unique because it came from `generateUniqueSlug`.)
     An explicit rename to the reserved slug `page` returns **`400`** (`"Reserved slug"`); a page already named `page` keeps saving in place.
  ...
  4. Writes the page under the final slug and, on a slug change, deletes the old file (via the shared persist helper). When the request carries `widgets`, the pagination rules run before the write (see Save content) and the anchor sweep after it, returning `listingAnchorMovedFrom`.
- **Save content (`savePageContent`)**: Persists widgets/SEO from the visual editor. Validates that `slug`, `name`, and `widgets` are present (`400` otherwise), then enforces the **per-page widget cap**: it counts `max(widgetsOrder.length, Object.keys(widgets).length)` and compares it against `LIMIT_KEYS.MAX_WIDGETS_PER_PAGE` from the limits adapter (falling back to the `MAX_WIDGETS_PER_PAGE` constant in `@widgetizer/core/adapters`, currently `5000`). Over the cap → **`422`**. OSS limits are effectively unbounded (`Infinity`), so the cap only bites on a finite hosted limit. Existing timestamps and `uuid` are preserved. A slug change to `page` returns `400`. Before writing, `enforcePaginationRules` checks widgets with `paginate` on: items per page (the value, else the schema default) must be a whole number ≥ 1 and only one widget per page may paginate (`422` otherwise); a widget whose schema can't paginate has `paginate` switched off; a paginating widget is given `listing_anchor`. After the write, `clearListingAnchorsElsewhere` clears `listing_anchor` and `paginate` on other pages for the collections this page claims — see [Collections §5a–5b](core-collections.md).
...
- **Delete (`deletePage`)**: Reads the page first to capture its `uuid` (missing → `404`), deletes the file, removes it from media usage tracking, then runs **automatic reference cleanup**: `cleanupDeletedPageReferences` clears every menu item, widget `link` setting, and richtext stable-link anchor across the project that referenced the deleted page's `uuid` (structured links emptied + `pageUuid` removed; richtext anchors carrying a matching `data-page-uuid` are unwrapped to plain text) — covering page widgets, global widgets (header/footer), collection items, and all menus.
...
- **Duplicate (`duplicatePage`)**: Reads the source page, lists existing page names, and derives the copy name with `generateCopyName` (`utils/namingHelpers.js`) — `Page Name (Copy)`, then `Page Name (Copy 2)`, `(Copy 3)`, … as collisions accumulate. The **new slug is derived from that copy name** via `generateUniqueSlug` (e.g. `about-us` → `about-us-copy` for `"About Us (Copy)"`), **a new UUID is generated** so the duplicate is a distinct entity, and the new file is written and returned `201`. Media usage is tracked for the copy. `listing_anchor` and `paginate` are cleared on the copy's widgets.

...
**Static file serving**: Screenshots are served from the authenticated, user-scoped themes directory via `/themes/*` (backed by `getUserThemesDir(req.userId)` in `server/createApp.js`). Preset screenshots at `/themes/arch/presets/restaurant/screenshot.png` work automatically for the authenticated user.

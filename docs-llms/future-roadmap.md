...
> **Status: order decided 2026-09-09, revised 2026-09-15. Stages 0–3 and 5–7 shipped on branch `0.9.10` (groundwork and breadcrumbs 2026-09-12, pagination 2026-09-14, structured data, undo history `e4379a50` and the "Widgetizer Desktop" rename `8fae65c5` 2026-09-15).** Revised order: stages 5 → 6 → 7 first, then the open minor fixes and hands-on testing, then multilang (stage 4) from a clean slate. Stage 7 (upload file names) was built 2026-09-16. **Multilang started 2026-09-16 and is in its last phase: steps 0–21 are in (phases 0–4 done, latest `1775a245`), leaving step 22 (Arch switcher and site-facing copy), 23 (localized month names), 24 (upgrade verification) and 25 (docs).** Hands-on testing began 2026-09-18 on a two-language test project. Nothing is urgent; each stage is finished and shipped before the next starts, and each lands groundwork the later ones use instead of rewriting. This page is the entry point — start here, then open the stage's own doc.

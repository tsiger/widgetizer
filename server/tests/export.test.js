...
const PROJECT_ID = "export-test-uuid-1234";
...
describe("exportProject validation", () => {
  ...
  it("returns 400 when project has no index page", async () => {
    ...
    assert.equal(res._status, 400);
    assert.ok(res._json.error.toLowerCase().includes("homepage"), `Expected homepage error, got: ${res._json.error}`);

    // Cleanup
    await fs.remove(projDir);
    const data2 = await readProjectsFile();
    data2.projects = data2.projects.filter((p) => p.id !== tempProjectId);
    await writeProjectsFile(data2);
  });
});

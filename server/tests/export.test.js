...
const PROJECT_ID = "export-test-uuid";
const PROJECT_FOLDER = "export-test-project";
...
async function cleanExportHistory() {
  ...
  for (const entry of publishContents) {
    if (
      ...
      entry.startsWith("cleanup-test") ||
      entry.startsWith("no-url") ||
      entry.startsWith("home-slug") ||
      entry.startsWith("bad-theme")
    ) {
...
describe("exportProject validation", () => {
  ...
  it("returns 400 when project has no index page", async () => {
    ...
    const tempProjectId = "no-index-project-uuid";
    const tempFolder = "no-index-project";
    const currentData = await projectRepo.readProjectsData();
    ...
    await projectRepo.writeProjectsData(currentData);
...
describe("exportProject without siteUrl", () => {
  const NO_URL_ID = "no-url-project-uuid";
  const NO_URL_FOLDER = "no-url-project";
...
describe("getExportFiles", () => {
  ...
  before(async () => {
    ...
    testExportDir = "test-entry-files";
  ...
  it("prefers home.html when index.html is absent", async () => {
    const noIndexDir = "test-home-entry";
  ...
  it("falls back to first HTML file when no index/home exists", async () => {
    const noSpecialDir = "test-fallback-entry";
...
describe("downloadExport", () => {
  ...
  before(async () => {
    testExportDir = "test-download-dir";
    const fullDir = path.join(PUBLISH_DIR, testExportDir);
    await fs.ensureDir(fullDir);
...
describe("cleanupProjectExports", () => {
  const CLEANUP_ID = "cleanup-test-uuid";
  const CLEANUP_FOLDER = "cleanup-test-project";
  ...
  it("removes all export directories for a project", async () => {
    ...
    const result = await cleanupProjectExports(CLEANUP_ID);
...
describe("home page slug mapping", () => {
  const HOME_SLUG_ID = "home-slug-uuid";
  const HOME_SLUG_FOLDER = "home-slug-project";
...
describe("export failure recording", () => {
  ...
  it("records a failed export in history when theme is missing", async () => {
    ...
    const badId = "bad-theme-uuid";
    const badFolder = "bad-theme-project";
    const data = await projectRepo.readProjectsData();
    data.projects.push({
      id: badId,

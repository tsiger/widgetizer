...
for (const TEST_USER_ID of TEST_USER_IDS) {
  describe(`[userId=${TEST_USER_ID}]`, () => {
    const PROJECT_ID = `preview-test-uuid-${TEST_USER_ID}`;
    const PROJECT_FOLDER = `preview-test-project-${TEST_USER_ID}`;
    ...
    describe("getGlobalWidgets", () => {
      const globalDir = () => path.join(getProjectDir(PROJECT_FOLDER, TEST_USER_ID), "pages", "global");

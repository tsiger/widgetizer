...
// ============================================================================
// Tests for each userId
// ============================================================================

for (const TEST_USER_ID of TEST_USER_IDS) {
  describe(`[userId=${TEST_USER_ID}]`, () => {
    const PROJECT_ID = `update-svc-uuid-${TEST_USER_ID}`;
    const PROJECT_FOLDER = `update-svc-project-${TEST_USER_ID}`;
    const THEME_NAME = `test-update-theme-${TEST_USER_ID}`;
    ...
    async function createProject(themeVersion) {
      ...
      // Project theme.json
      await fs.writeJson(
        getProjectThemeJsonPath(PROJECT_FOLDER, TEST_USER_ID),
        {
          name: "Test Update Theme",
    ...
    describe("checkForUpdates", () => {
      ...
      it("detects when update is available", async () => {
        ...
        await buildLatestSnapshot(THEME_NAME, TEST_USER_ID);

        // Project still at v1.0.0
        await createProject("1.0.0");

        const result = await checkForUpdates(PROJECT_ID, TEST_USER_ID);
    ...
    describe("applyThemeUpdate", () => {
      ...
      it("copies updatable paths from theme to project", async () => {
        ...
        const result = await applyThemeUpdate(PROJECT_ID, TEST_USER_ID);
    ...
    describe("toggleThemeUpdates", () => {
      ...
      it("enables theme update notifications", async () => {
        const result = await toggleThemeUpdates(PROJECT_ID, true, TEST_USER_ID);
        assert.ok(result.success);
      ...
      it("disables theme update notifications", async () => {
        const result = await toggleThemeUpdates(PROJECT_ID, false, TEST_USER_ID);
        assert.ok(result.success);
      ...
      it("throws for nonexistent project", async () => {
        await assert.rejects(
          () => toggleThemeUpdates("fake-project-uuid", true, TEST_USER_ID),
          (err) => {
            assert.ok(err.message.includes("not found"));
            return true;
          },

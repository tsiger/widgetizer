...
// ============================================================================
// Cleanup after ALL user-scoped test suites
// ============================================================================
...
// ============================================================================
// Tests — run for each userId
// ============================================================================

for (const TEST_USER_ID of TEST_USER_IDS) {
  describe(`[userId=${TEST_USER_ID}]`, () => {
    const TEST_THEME_ID = `__test_theme_updates_${TEST_USER_ID}__`;
    const testThemeDir = getThemeDir(TEST_THEME_ID, TEST_USER_ID);
    ...
    // ========================================================================
    // Build the snapshot once before all tests in this describe block
    // ========================================================================

    before(async () => {
      await setup();
      await buildLatestSnapshot(TEST_THEME_ID, TEST_USER_ID);
    });
    ...
    describe("Version detection", () => {
      it("detects all versions (base + updates)", async () => {
        const versions = await getThemeVersions(TEST_THEME_ID, TEST_USER_ID);
    ...
    describe("Error handling", () => {
      it("throws when update folder is missing theme.json", async () => {
        const errorThemeId = `__test_theme_error_missing_${TEST_USER_ID}__`;
        const errorThemeDir = getThemeDir(errorThemeId, TEST_USER_ID);
        ...
        try {
          ...
          await assert.rejects(
            () => buildLatestSnapshot(errorThemeId, TEST_USER_ID),
            (err) => {
      ...
      it("throws when theme.json version doesn't match folder name", async () => {
        const errorThemeId = `__test_theme_error_mismatch_${TEST_USER_ID}__`;
        const errorThemeDir = getThemeDir(errorThemeId, TEST_USER_ID);
      ...
      it("does not create latest/ when no updates exist", async () => {
        const baseOnlyId = `__test_theme_base_only_${TEST_USER_ID}__`;
        const baseOnlyDir = getThemeDir(baseOnlyId, TEST_USER_ID);

        try {
          await fs.ensureDir(baseOnlyDir);
          await fs.writeJson(path.join(baseOnlyDir, "theme.json"), { name: "Base Only", version: "1.0.0" });

          await buildLatestSnapshot(baseOnlyId, TEST_USER_ID);

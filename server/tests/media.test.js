...
for (const TEST_USER_ID of TEST_USER_IDS) {
  describe(`[userId=${TEST_USER_ID}]`, () => {
    // Per-user unique IDs to avoid DB PRIMARY KEY conflicts
    const PROJECT_ID = `media-test-uuid-${TEST_USER_ID}`;
    const PROJECT_FOLDER = `media-test-project-${TEST_USER_ID}`;
    ...
    before(async () => {
      ...
      await fs.ensureDir(getProjectImagesDir(PROJECT_FOLDER, TEST_USER_ID));
      await fs.ensureDir(getProjectVideosDir(PROJECT_FOLDER, TEST_USER_ID));
      await fs.ensureDir(getProjectAudiosDir(PROJECT_FOLDER, TEST_USER_ID));
    });
    ...
    describe("readMediaFile", () => {
      ...
      it("returns empty files array when no media exists", async () => {
        const data = await readMediaFile(PROJECT_ID, TEST_USER_ID);
      ...
      it("throws when project does not exist", async () => {
        await assert.rejects(
          () => readMediaFile("nonexistent-project-uuid", TEST_USER_ID),
          (err) => {
    ...
    describe("writeMediaFile", () => {
      it("writes media data", async () => {
        ...
        await writeMediaFile(PROJECT_ID, testData, TEST_USER_ID);

        const saved = await readMediaFile(PROJECT_ID, TEST_USER_ID);
        assert.equal(saved.files.length, 1);
        assert.equal(saved.files[0].id, "write-1");
      });

      it("overwrites existing data", async () => {
        await writeMediaFile(PROJECT_ID, { files: [{ id: "old" }] }, TEST_USER_ID);
        await writeMediaFile(PROJECT_ID, { files: [{ id: "new-1" }, { id: "new-2" }] }, TEST_USER_ID);
      ...
      it("handles concurrent writes safely (no data corruption)", async () => {
        ...
        const writes = [];
        for (let i = 0; i < 5; i++) {
          writes.push(writeMediaFile(PROJECT_ID, { files: [{ id: `concurrent-${i}` }] }, TEST_USER_ID));
        }
    ...
    describe("uploadProjectMedia", () => {
      beforeEach(async () => {
        ...
        await fs.emptyDir(getProjectImagesDir(PROJECT_FOLDER, TEST_USER_ID));
        await fs.emptyDir(getProjectVideosDir(PROJECT_FOLDER, TEST_USER_ID));
        await fs.emptyDir(getProjectAudiosDir(PROJECT_FOLDER, TEST_USER_ID));
      });
      ...
      it("uploads a JPEG image and processes with sharp", async () => {
        ...
        const imgDir = getProjectImagesDir(PROJECT_FOLDER, TEST_USER_ID);
      ...
      it("uploads a video file with basic metadata", async () => {
        const vidDir = getProjectVideosDir(PROJECT_FOLDER, TEST_USER_ID);
      ...
      it("uploads an audio file with basic metadata", async () => {
        const audDir = getProjectAudiosDir(PROJECT_FOLDER, TEST_USER_ID);
      ...
      it("persists uploaded files in media.json", async () => {
        ...
        const mediaData = await readMediaFile(PROJECT_ID, TEST_USER_ID);
    ...
    describe("theme image size overrides", () => {
      ...
      async function setThemeImageSizes(imageSizes) {
        const themeDir = getThemeDir(THEME_NAME, TEST_USER_ID);
        await fs.ensureDir(themeDir);
        const themeJson = { name: "Media Test Theme", version: "1.0.0" };
        if (imageSizes !== undefined) {
          themeJson.settings = { imageSizes };
        }
        await fs.writeJson(getThemeJsonPath(THEME_NAME, TEST_USER_ID), themeJson, { spaces: 2 });
      }

      /** Helper: clean the theme directory. */
      async function removeTheme() {
        await fs.remove(getThemeDir(THEME_NAME, TEST_USER_ID));
      }

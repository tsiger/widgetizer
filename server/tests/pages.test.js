...
for (const TEST_USER_ID of TEST_USER_IDS) {
  describe(`[userId=${TEST_USER_ID}]`, () => {
    const PROJECT_ID = `page-test-project-uuid-${TEST_USER_ID}`;
    const PROJECT_FOLDER = `page-test-project-${TEST_USER_ID}`;
    ...
    async function resetPages() {
      const pagesDir = getProjectPagesDir(activeProject.folderName, TEST_USER_ID);
    ...
    before(async () => {
      ...
      const themeDir = getThemeDir("__page_test_theme__", TEST_USER_ID);
    ...
    describe("createPage", () => {
      ...
      it("writes the page file to disk", async () => {
        ...
        const filePath = getPagePath(activeProject.folderName, page.slug, TEST_USER_ID);
    ...
    describe("listProjectPagesData", () => {
      ...
      it("returns page data directly by folderName", async () => {
        ...
        const pages = await listProjectPagesData(activeProject.folderName, TEST_USER_ID);
        assert.equal(pages.length, 1);
        assert.equal(pages[0].name, "Direct Access");
      });

      it("returns empty array for non-existent project", async () => {
        const pages = await listProjectPagesData("non-existent-folder", TEST_USER_ID);
    ...
    describe("readGlobalWidgetData", () => {
      it("reads header global widget", async () => {
        const header = await readGlobalWidgetData(activeProject.folderName, "header", TEST_USER_ID);
        assert.ok(header);
        assert.equal(header.type, "header");
      });

      it("reads footer global widget", async () => {
        const footer = await readGlobalWidgetData(activeProject.folderName, "footer", TEST_USER_ID);
        assert.ok(footer);
        assert.equal(footer.type, "footer");
      });

      it("returns null for invalid widget type", async () => {
        const result = await readGlobalWidgetData(activeProject.folderName, "sidebar", TEST_USER_ID);
        assert.equal(result, null);
      });

      it("returns null for non-existent project", async () => {
        const result = await readGlobalWidgetData("non-existent", "header", TEST_USER_ID);
        assert.equal(result, null);
      });
    });
    ...
    describe("updatePage", () => {
      ...
      it("renames file when slug changes", async () => {
        ...
        const oldPath = getPagePath(activeProject.folderName, page.slug, TEST_USER_ID);
        ...
        assert.ok(await fs.pathExists(getPagePath(activeProject.folderName, "renamed-slug", TEST_USER_ID)));
      });
      ...
      it("preserves widgets when not included in update body", async () => {
        ...
        const pagePath = getPagePath(activeProject.folderName, page.slug, TEST_USER_ID);
    ...
    describe("savePageContent", () => {
      ...
      it("saves page content with widgets", async () => {
        ...
        // Verify on disk
        const onDisk = await fs.readJson(getPagePath(activeProject.folderName, page.slug, TEST_USER_ID));
      ...
      it("handles slug change and deletes old file", async () => {
        ...
        assert.ok(await fs.pathExists(getPagePath(activeProject.folderName, "new-slug", TEST_USER_ID)));
      });
    ...
    describe("duplicatePage", () => {
      ...
      it("creates a file on disk for the duplicate", async () => {
        ...
        const dupPath = getPagePath(activeProject.folderName, res._json.slug, TEST_USER_ID);

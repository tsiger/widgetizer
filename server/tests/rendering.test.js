...
for (const TEST_USER_ID of TEST_USER_IDS) {
  describe(`[userId=${TEST_USER_ID}]`, () => {
    const PROJECT_ID = `render-test-uuid-${TEST_USER_ID}`;
    const PROJECT_FOLDER = `render-test-project-${TEST_USER_ID}`;
    ...
    before(async () => {
      ...
      await fs.ensureDir(getProjectMenusDir(PROJECT_FOLDER, TEST_USER_ID));
...
<body class="{{ body_class }}">
  ...
  {% if widget.settings.cta_link.href != blank %}<a href="{{ widget.settings.cta_link.href }}">{{ widget.settings.cta_link.text }}</a>{% endif %}
      ...
      // Create a test menu
      await fs.writeFile(
        path.join(getProjectMenusDir(PROJECT_FOLDER, TEST_USER_ID), "main-nav.json"),
        JSON.stringify(
          {
            id: "main-nav",
    ...
    describe("renderPageLayout", () => {
      ...
      it("returns error HTML when layout.liquid is missing", async () => {
        ...
        const noLayoutProject = {
          id: `no-layout-uuid-${TEST_USER_ID}`,
          folderName: `no-layout-project-${TEST_USER_ID}`,
          name: "No Layout",
          theme: "__no_layout__",
          created: new Date().toISOString(),
        };
        ...
        const noLayoutDir = getProjectDir(noLayoutProject.folderName, TEST_USER_ID);
